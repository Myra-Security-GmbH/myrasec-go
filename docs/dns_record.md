...
| `RecordType` | string | This value sets the type of the DNS record. All available record types are `A`, `AAAA`, `MX`, `CNAME`, `TXT`, `NS`, `SRV`, `PTR` and `DS`. `PTR` records are only useable on `.arpa` domains. |
...
| Field | Type | Description|
|---|---|---|
| `ID` | int | ID is an unique identifier for an object. This value is always a number type and cannot be set while inserting a new object. To update or delete a UpstreamOptions object, it is necessary to add this attribute to your object. |
