...
| --- | --- | --- |
| `ID` | int | ID is a unique identifier for an object. This value is always a number type and cannot be set while inserting a new object. To update or delete a TagWAFRule it is necessary to add this attribute to your object. |
...
## Create
To create a new TagWAFRule it is necessary to send a TagWAFRule object without the attributes "id" and "modified". Both attributes will be generated by the server and returned after a successful insert is done.
...
## Delete
For deleting a TagWAFRule it is only necessary to send the "id" and "modified" attribute as body content.
